...
export class StoreController {
  ...
  public static async login(req: Request, res: Response) {
    try {
      ...
      if (isStore) {
        res
          .status(STATUS_CODE.SUCCESS)
          .json(successAction(STATUS_CODE.SUCCESS, isStore, MESSAGE.LOGIN))
      } else if (isStore === 'invalidStore') {
        res
          .status(STATUS_CODE.SUCCESS)
          .json(failAction(STATUS_CODE.SUCCESS, isStore, MESSAGE.Invalidlogin))
      } else isStore === 'notExist'
      {
        res
          .status(STATUS_CODE.SUCCESS)
          .json(failAction(STATUS_CODE.SUCCESS, MESSAGE.notExist('Store')))
      }
    } catch (err: any) {
      res
        .status(STATUS_CODE.BAD_REQUEST)
  ...
  public static async getStoreById(req: Request, res: Response) {
    try {
      ...
      if (!findStore) {
        res
          .status(STATUS_CODE.NOT_CREATED)
          .json(
            failAction(
              STATUS_CODE.SUCCESS,
              MESSAGE.notExist('Store not existed'),
            ),
          )
      }
      ...
      res
        .status(STATUS_CODE.SUCCESS)
        .json(
          successAction(STATUS_CODE.SUCCESS, findStore, MESSAGE.fetch('Store')),
        )
    } catch (err: any) {
  ...
  public static async getAllStore(req: Request, res: Response) {
    try {
      ...
      res
        .status(STATUS_CODE.SUCCESS)
        .json(
          successAction(STATUS_CODE.SUCCESS, storeData, MESSAGE.fetch('Store')),
        )
    } catch (err: any) {
    ...
    try {
      ...
      if (data) {
        res
          .status(STATUS_CODE.SUCCESS)
          .json(
            successAction(STATUS_CODE.SUCCESS, data, MESSAGE.update('Store')),
          )
      }
    } catch (err: any) {
  ...
  public static async deleteStore(req: Request, res: Response) {
    try {
      ...
      res
        .status(STATUS_CODE.SUCCESS)
        .json(successAction(STATUS_CODE.SUCCESS, MESSAGE.delete('Store')))
    } catch (err: any) {
